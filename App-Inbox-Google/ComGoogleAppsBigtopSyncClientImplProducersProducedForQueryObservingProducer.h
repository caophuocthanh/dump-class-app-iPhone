//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "ComGoogleAppsBigtopSyncClientImplProducersWrappingElementProducer.h"

@class ComGoogleAppsBigtopSyncClientImplProducersQueryMetadata;

@interface ComGoogleAppsBigtopSyncClientImplProducersProducedForQueryObservingProducer : ComGoogleAppsBigtopSyncClientImplProducersWrappingElementProducer
{
    ComGoogleAppsBigtopSyncClientImplProducersQueryMetadata *queryMetadata_;
    id <JBTICommonListener> listener_;
}

+ (const struct J2ObjcClassInfo *)__metadata;
+ (id)__annotations_listener_;
- (void)dealloc;
- (void)setListenerWithJBTICommonListener:(id)arg1;
- (void)stop;
- (void)handleChangesWithComGoogleAppsBigtopSyncClientImplProducersElementProducer:(id)arg1 withComGoogleAppsBigtopSyncClientImplProducersElementProducer_ChangeList:(id)arg2;
- (id)initWithComGoogleAppsBigtopSyncClientImplProducersElementProducer:(id)arg1;

@end

