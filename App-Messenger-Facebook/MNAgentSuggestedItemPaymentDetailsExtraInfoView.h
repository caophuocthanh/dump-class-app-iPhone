//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UIView.h"

#import "MNPaymentsTransferReceiptExtraInfoView.h"

@class FBRichTextView, MNAgentSuggestedItemPaymentDetailsExtraInfoViewModel, MNCommerceItemView, NSString;

@interface MNAgentSuggestedItemPaymentDetailsExtraInfoView : UIView <MNPaymentsTransferReceiptExtraInfoView>
{
    MNCommerceItemView *_itemView;
    FBRichTextView *_statusTitleTextView;
    FBRichTextView *_statusTextView;
    FBRichTextView *_paymentMethodTitleTextView;
    FBRichTextView *_paymentMethodTextView;
    MNAgentSuggestedItemPaymentDetailsExtraInfoViewModel *_viewModel;
}

@property(copy, nonatomic) MNAgentSuggestedItemPaymentDetailsExtraInfoViewModel *viewModel; // @synthesize viewModel=_viewModel;
- (void).cxx_destruct;
- (void)_applicationWillResignActive:(id)arg1;
- (void)_applicationDidBecomeActive:(id)arg1;
- (void)layoutSubviews;
- (struct CGSize)sizeThatFits:(struct CGSize)arg1;
- (void)dealloc;
- (id)initWithFrame:(struct CGRect)arg1 imageDownloader:(id)arg2;
- (id)initWithFrame:(struct CGRect)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned int hash;
@property(readonly) Class superclass;

@end

