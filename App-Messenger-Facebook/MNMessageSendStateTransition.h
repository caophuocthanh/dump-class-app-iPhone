//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "FBValueObject.h"

#import "NSCopying.h"

@class MNMessageSendState;

@interface MNMessageSendStateTransition : FBValueObject <NSCopying>
{
    MNMessageSendState *_fromState;
    MNMessageSendState *_toState;
}

@property(readonly, copy, nonatomic) MNMessageSendState *toState; // @synthesize toState=_toState;
@property(readonly, copy, nonatomic) MNMessageSendState *fromState; // @synthesize fromState=_fromState;
- (void).cxx_destruct;
- (id)initWithFromState:(id)arg1 toState:(id)arg2;

@end

